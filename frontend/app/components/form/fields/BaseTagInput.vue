...
function handleKey(event: KeyboardEvent) {
  ...
  if (event.key === "Enter") {
    // manual check if enter was the button that was pressed
...
<template>
  <div class="m-4">
    ...
    <input
      ...
      v-model="inputText"
      @keydown="handleKey"
      class="px-4 bg-muted border border-border h-12 font-bold uppercase text-[10px] tracking-widest rounded-lg w-full outline-none hover:border-foreground/20 hover:bg-muted/70 focus:border-foreground/30 focus:bg-background placeholder:text-muted-foreground placeholder:opacity-100 dark:placeholder:opacity-90"
    />
