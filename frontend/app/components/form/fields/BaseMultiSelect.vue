...
function handleSelect(value: string) { // handles selecting an item
  if (!value) return
...
function removeItem(item: string) { // handles removing an item
  ...
  <div class="m-4">
    ...
    <div v-if="model.length" data-testid="item-container" class="mt-2 border border-border rounded-lg overflow-hidden"> <!-- testid to make it easier for testing -->
      ...
      >
        ...
        <X class="w-3 h-3 shrink-0 cursor-pointer text-muted-foreground hover:text-foreground transition-colors" @click="removeItem(item)" />
      </div>
    </div>
  </div>
</template>
