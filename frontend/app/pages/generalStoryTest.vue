...
const fetchPage = async (reset = false) => {
  ...
  try {
    const result = await getAll(
      ...
      undefined,
      sortOrder.value === "newest", // descending
    );
    const raw        = (result as unknown) as PaginatedResponse<Blog | BlogView>;
    const objects    = raw?.objects ?? [];
    totalItems.value = raw?.totalItems ?? 0;
...
// The sentinel lives inside `v-else`, so it is not in the DOM during onMounted.
// Watch the ref and attach the observer the moment it becomes available.
...
// Random ISO timestamp between 2016-01-01 and now
const randomDate = (): string => {
  const start = new Date("2016-01-01T00:00:00Z").getTime();
  return new Date(start + Math.random() * (Date.now() - start)).toISOString();
};

// ── Mock stories — stored in global Nuxt state so [id].vue can read them ──
// useState key must be stable across navigations.
const mockStories = useState<Array<Blog>>("mock-stories", () => []);

// Negative IDs avoid collisions with real DB ids; counter lives outside state
// so it keeps decrementing within the session.
let mockIdCounter = -1;
  ...
  <div class="min-h-screen bg-background text-foreground">
      ...
      <div class="container mx-auto px-4 max-w-4xl py-3 flex items-center gap-2">
        ...
        >
          ...
          <span
            ...
            class="text-[8px] bg-foreground text-background px-1.5 py-0.5 rounded-full leading-none"
          >
            {{ mockStories.length }}
          </span>
        </button>
    ...
    <div class="container mx-auto px-4 max-w-4xl py-12">
          ...
          <div class="space-y-8">
            <div v-for="k in 2" :key="k">
              <div class="flex items-center gap-3 mb-3">
                <div class="h-3 w-24 bg-muted animate-pulse" />
                <div class="flex-1 h-px bg-muted" />
              </div>
      ...
      <div v-else-if="error" class="py-24 text-center">
        ...
        <p class="font-brand font-black text-[10px] uppercase tracking-widest text-red-500 mb-8">{{ error.message }}</p>
        <button
      ...
      <template v-else>
        ...
        <!-- End of list -->
        <div v-else-if="!hasMore && allStories.length > 0" class="flex items-center gap-4 py-10">
