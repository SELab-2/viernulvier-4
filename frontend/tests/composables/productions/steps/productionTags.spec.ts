...
describe("useProductionTags", () => {
  ...
  it("resets to original state", async () => {
    ...
    draft.value.push({ type: "new", label: { nl: "New", en: "New" } });
    reset();
  ...
  it("extracts payload correctly", async () => {
    ...
    expect(payload.create).toEqual([{ nl: "T3", en: "T3" }]);
  });
});
