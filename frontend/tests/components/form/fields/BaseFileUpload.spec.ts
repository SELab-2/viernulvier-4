...
const createFile = (
  name: string,
  type: string, // helper function to create a file with a dummy string within
...
describe("BaseFileUpload", () => {
  ...
  it("replaces file in single mode when a new file is selected", async () => {
    ...
    Object.defineProperty(input.element, "files", {
      value: [file1],
      configurable: true,
    }); // simulate user selecting file1 in the file picker
    await input.trigger("change");

    Object.defineProperty(input.element, "files", {
      value: [file2],
      configurable: true,
    }); // simulate user selecting file2 in the file picker
    await input.trigger("change");
