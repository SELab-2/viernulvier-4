...
export class InjectCsvEngine {
  ...
  async injectProductionsCSV(input: CsvInputSource) {
    ...
    await this.dbConnection.insertProductions(translatedProductions);
  ...
  async injectEventsCSV(input: CsvInputSource) {
    ...
    await this.dbConnection.insertLocations(translatedLocations);
    ...
    await this.dbConnection.insertEvents(translatedEvents);
  ...
  async injectTagsCSV(input: CsvInputSource) {
    ...
    for (const row of parsedTags) {
      ...
      const tagId = await this.dbConnection.insertTag({
  ...
  async injectBlogsCSV(input: CsvInputSource) {
    ...
    for (const row of parsedBlogs) {
      ...
      try {
        ...
        const blog = await this.dbConnection.insertBlog(
  ...
  async injectPricesCSV(input: CsvInputSource) {
    ...
    for (const row of parsedPrices) {
      ...
      try {
        const event = await this.dbConnection.getEventByLegacyId(eventLegacyId);

        const priceLegacyId = "csv-" + translatedPriceData.name.nl;

        const price = await this.dbConnection.insertPrice({
  ...
  async injectOldCsvData() {
    ...
    await Promise.all([
      this.dbConnection.insertTags(translatedTags),
      this.dbConnection.insertLocations(translatedLocations),
    ]);
